...
public:
	...
	Keybind       thirdperson;
	Slider        thirdpersondist;
	Checkbox      angle_indicator;
	Dropdown      hitmarker_type;
...
public:
	void init( ) {
		...
		thirdpersondist.setup( XOR( "distance" ), XOR( "distance" ), 0, 1000, true, 0, 150, 1, XOR(L"u"));
		RegisterElement(&thirdpersondist);

		angle_indicator.setup(XOR("angle ind"), XOR("imtoolazyits553amplshelp"));
		RegisterElement(&angle_indicator);

		hitmarker_type.setup(XOR("hitmarker type"), XOR("hitmarker_type"), {XOR("static"), XOR("world")});
		RegisterElement(&hitmarker_type);
	}
};

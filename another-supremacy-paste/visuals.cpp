...
void Visuals::ThirdpersonThink( ) {
	...
	if( g_csgo.m_input->CAM_IsThirdPerson( ) ) {
		...
		offset.z = g_menu.main.visuals.thirdpersondist.get();
...
void Visuals::Hitmarker( ) {
	...
	if (g_menu.main.visuals.hitmarker_type.get() != 0)
		return;
...
/*void Visuals::WorldHitmarker() {

	if (g_menu.main.visuals.hitmarker_type.get() != 1)
		return;
	...
	for (size_t i{ }; i < m_world_hitmarkers.size(); ++i) {
		...
		if (g_inputpred.m_curtime - info.m_time > 2.0f) {
		...
		if (info.m_world_to_screen) {
			...
			auto draw_lines = [&](vec2_t pos_custom, Color clr) -> void {
				auto linesize = 8;
				g_csgo.m_surface->DrawLine(pos_custom.x - linesize, pos_custom.y - linesize, pos_custom.x - (linesize / 4), pos_custom.y - (linesize / 4)),
				g_csgo.m_surface->DrawSetColor(255 * info.m_alpha);
				g_csgo.m_surface->DrawLine(pos_custom.x - linesize, pos_custom.y + linesize, pos_custom.x - (linesize / 4), pos_custom.y + (linesize / 4)),
				g_csgo.m_surface->DrawSetColor(255 * info.m_alpha);
				g_csgo.m_surface->DrawLine(pos_custom.x + linesize, pos_custom.y + linesize, pos_custom.x + (linesize / 4), pos_custom.y + (linesize / 4)),
				g_csgo.m_surface->DrawSetColor(255 * info.m_alpha);
			...
			// hitmarker
draw_lines(info.m_world_pos, colors::white);
		}
	}
}*/

void Visuals::AAIndicator( ) {
	//auto local_player = static_cast<Player*>(g_csgo.m_entitylist()->GetClientEntity(g_csgo.m_engine()->GetLocalPlayer()));
	auto local_player = g_cl.m_local;

	static auto degrees_to_radians = math::deg_to_rad;

	auto rotated_position = [](vec3_t start, const float rotation, const float distance) -> vec3_t {
	...
	if (g_menu.main.visuals.angle_indicator.get()) {
		auto client_viewangles = g_cl.m_view_angles;
		auto
			screen_width = g_cl.m_width,
			screen_height = g_cl.m_height;

		g_csgo.m_engine->GetViewAngles(client_viewangles);

		constexpr auto radius = 80.f;

		const auto screen_center = vec2_t(screen_width / 2.f, screen_height / 2.f);
		const auto real_rot = degrees_to_radians(client_viewangles.y - g_cl.m_abs_yaw - 90);
		const auto fake_rot = degrees_to_radians(g_cl.m_cmd->m_view_angles.y - 90);
		const auto lby_rot = degrees_to_radians(client_viewangles.y - g_cl.m_local->m_flLowerBodyYawTarget() - 90);

		auto draw_arrow = [&](float rot, Color color) -> void {
			auto pos_one = vec2_t(screen_center.x + cosf(rot) * radius, screen_center.y + sinf(rot) * radius);
			auto pos_two = vec2_t(screen_center.x + cosf(rot + degrees_to_radians(8)) * (radius - 13.f),
				screen_center.y + sinf(rot + degrees_to_radians(8)) * (radius - 13.f));
			auto pos_three = vec2_t(screen_center.x + cosf(rot - degrees_to_radians(8)) * (radius - 13.f),
				screen_center.y + sinf(rot - degrees_to_radians(8)) * (radius - 13.f));

			render::triangle(
				vec2_t(pos_one.x, pos_one.y),
				vec2_t(pos_two.x, pos_two.y),
				vec2_t(pos_three.x, pos_three.y),
				color);
		};

...
void HVH::DoRealAntiAim( ) {
	...
	if( m_yaw > 0 ) {
		...
		// run normal aa code.
			...
			case 3: {
				// set base angle.
				g_cl.m_cmd->m_view_angles.y = ( m_direction - m_rot_range );

				// apply spin.
				g_cl.m_cmd->m_view_angles.y += std::fmod( g_csgo.m_globals->m_curtime * ( m_rot_speed * 30000000 ), m_rot_range * 30000000 );

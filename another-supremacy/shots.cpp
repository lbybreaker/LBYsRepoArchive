...
void Shots::OnImpact( IGameEvent *evt ) {
	...
	// check if the resolver missed, shit way to do this, but it works
	if ( +data->m_missed_shots && trace.m_entity == target ) {

		if ( g_menu.main.misc.notifications.get( 2 ) ){
		
			auto mode = shot->m_record->m_mode;
			/*int damage;

			damage = evt->m_keys->FindKey(HASH("dmg_health"))->GetInt();

			if ( damage < 0 )
				return;*/

			// still prints if people are using god, don't know how to fix
			// tried grabbing the damage value, it just crashes

			if (mode == Resolver::Modes::RESOLVE_STAND) {
				g_notify.add(XOR("missed shot due to resolver. [ mode: STAND ]\n"), colors::white, colors::red);
			}

			if (mode == Resolver::Modes::RESOLVE_STAND1) {
				g_notify.add(XOR("missed shot due to resolver. [ mode: STAND1 ]\n"), colors::burgundy);
			}

			if (mode == Resolver::Modes::RESOLVE_STAND2) {
				g_notify.add(XOR("missed shot due to resolver. [ mode: STAND2 ]\n"), colors::burgundy);
			}

			if (mode == Resolver::Modes::RESOLVE_WALK) {
				g_notify.add(XOR("missed shot due to resolver. [ mode: WALK ]\n"), colors::burgundy);
			}

			if ( mode == Resolver::Modes::RESOLVE_STOPPED_MOVING ) {
				g_notify.add(XOR("missed shot due to resolver. [ mode: STOPPED ]\n"), colors::burgundy);
			}

			if (mode == Resolver::Modes::RESOLVE_BODY) {
				g_notify.add(XOR("missed shot due to resolver. [ mode: BODY ]\n"), colors::burgundy);
			}

			if ( mode == Resolver::Modes::RESOLVE_AIR ) {
				g_notify.add(XOR("missed shot due to resolver. [ mode: AIR ]\n"), colors::burgundy);
			}
...
void Shots::OnHurt( IGameEvent *evt ) {
	...
	if ( g_menu.main.misc.notifications.get( 1 ) ) {
		std::string out = tfm::format( XOR( "hit %s for %i in the %s (%i remaining)\n" ), name, ( int )damage, m_groups[group], hp );
		g_notify.add( out );
	}

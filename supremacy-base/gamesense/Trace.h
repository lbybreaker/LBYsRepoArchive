...
public:
	__forceinline surfacedata_t* GetSurfaceData( int surfaceDataIndex ) {
		return Utilities::GetMethod< surfacedata_t*( __thiscall* )( decltype( this ), int ) >( this, 5 )( this, surfaceDataIndex );
	}
};
...
public:
	__forceinline int GetPointContents( const vec3_t& position, int mask = MASK_ALL, void** entity = nullptr ) {
		return Utilities::GetMethod< int( __thiscall* )( decltype( this ), const vec3_t&, int, void** ) >( this, GETPOINTCONTENTS )( this, position, mask, entity );
	}

	__forceinline void ClipRayToEntity( const Ray& ray, uint32_t mask, Entity* entity, CGameTrace* trace ) {
		return Utilities::GetMethod< void( __thiscall* )( decltype( this ), const Ray&, uint32_t, Entity*, CGameTrace* ) >( this, CLIPRAYTOENTITY )( this, ray, mask, entity, trace );
	}

	__forceinline void TraceRay( const Ray& ray, uint32_t mask, ITraceFilter* filter, CGameTrace* trace ) {
		return Utilities::GetMethod< void( __thiscall* )( decltype( this ), const Ray&, uint32_t, ITraceFilter*, CGameTrace* ) >( this, TRACERAY )( this, ray, mask, filter, trace );
	}
};
...
bool Aimbot::CanDT() {
	...
	if (g_movement.m_walk_mode == 0)
		return true;

	if (g_movement.m_walk_mode == 1)
		return false;
...
// sv_cheats 1; bot_stop 1; bot_chatter off; mp_warmup_end; mp_ignore_round_win_conditions 1;
	...
	//	*g_cl.m_packet = false;
		*g_cl.m_packet = true;
		...
		if (m_target) {
			...
			if (g_menu.main.aimbot.shot_chams.get())
				g_visuals.DrawHitboxMatrix(m_record, g_menu.main.aimbot.shot_chams_col.get(), 10.f);
		}

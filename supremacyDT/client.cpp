...
void Client::DrawHUD() {
	...
	std::string text = tfm::format(XOR("supremacy | ping: %n"), g_cl.m_user, ms);
	render::FontSize_t size = render::menu.size(text);

	render::rect_filled(m_width - size.m_width - 15, 10, size.m_width + 10, size.m_height + 3, { 30, 30, 30, 255 });
	render::gradient(m_width - size.m_width - 15, 10, size.m_width + 10, size.m_height + 3, { 0, 165, 255, 120 }, { 0, 165, 255, 0 } );
	render::rect_filled(m_width - size.m_width - 15, 10, size.m_width + 10, 1, { 0, 165, 255, 255 });
	...
	*/
	...
	std::string text = tfm::format( XOR("supremacy | ping: %ims | tick: %i "), ms, rate );
	...
	render::gradient(m_width - size.m_width - 20, 10, size.m_width + 10, size.m_height + 2, color, { 0, 0, 0, 0 });

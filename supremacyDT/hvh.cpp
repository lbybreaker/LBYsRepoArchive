...
void HVH::SendPacket() {
	...
	if ( g_aimbot.CanDT() && g_menu.main.aimbot.doubletap.get() && !m_fake_duck ) {
	...
	if ( g_movement.m_walk_mode == 1 && g_input.GetKeyState(g_menu.main.movement.fakewalk.get()) ) {
		*g_cl.m_packet = false;
	}

	// do not lag while shooting.
	if (!g_hvh.m_fake_duck && g_menu.main.config.mode.get()) {
		if (g_cl.m_old_shot)
			*g_cl.m_packet = true;
	}

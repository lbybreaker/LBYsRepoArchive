...
void Movement::SlowWalk() {
	...
	m_walk_mode = 0;
		...
		if (g_cl.m_weapon_info && g_input.GetKeyState(g_menu.main.movement.fakewalk.get())) {

		// get the max possible speed whilest we are still accurate.
		float flMaxSpeed = g_cl.m_local->m_bIsScoped() > 1 ? g_cl.m_weapon_info->m_max_player_speed : g_cl.m_weapon_info->m_max_player_speed_alt;
...
void Movement::MoonWalk(CUserCmd* cmd) {
	...
	if (g_menu.main.misc.slidewalk.get()) {
		if (cmd->m_side_move < 0.f)
		{
			cmd->m_buttons |= IN_MOVERIGHT;
			cmd->m_buttons &= ~IN_MOVELEFT;
